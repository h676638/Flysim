...
function FLYSIM
    ...
    s2 = []; % Surface 2
    pe = 0;  % Engine Sound  
    [es1,fs1] = audioread('alarm.mp3');
    alarm = audioplayer(es1,fs1); % Alarm sound
    fig = figure;
    hold on;
    ...
    [sufFlat] = AddSurface();
    [s1, s2, s3] = AddIslands();
    btn = uicontrol("Style", "pushbutton", "String", "Click me", "Position", [150, 100, 100, 40]);
    set(btn, "callback", @ChangeScenery);
    ...
    function [s1,s2,s3] = AddIslands
        ...
        z = z * 1000;
        s3=surf(x,y,z, ...
               'LineStyle','none','AmbientStrength',0.1);
        s3.FaceColor = 'texturemap';
        s3.CData = textureIce;    
    end
%% Changes scenery when button is pressed
    function ChangeScenery(a, b)
        switch curScenery
            case "sea"
                textureSea = imread("desert.jpg");
                islandTexture = imread("mountain.jpg");
                curScenery = "desert";
                s3.CData = islandTexture;
                s2.CData = islandTexture;
                s1.CData = islandTexture;
            case "desert"
                textureSea = imread("ice.jpg");
                islandTexture = imread("ice.jpg");
                curScenery = "ice";
                s3.CData = islandTexture;
                s2.CData = islandTexture;
                s1.CData = islandTexture;
            case "ice"
                textureSea = imread("Sea.jpg");
                curScenery = "sea";
                s3.CData = imread("ice.jpg");
                s2.CData = imread("forrest.jpg");
                s1.CData = imread("desert.jpg");
        end
    ...
    function UpdateFuel
        if (kwt < 0)
            ...
            height = pos(3);
            heightIncrement = heightIncrement + x;
    ...
    %% Warn if altitude is 200 or less
    function Alarm()
        z = pos(3) - 200;
        if z < 0 || z < GetZ(s1, pos) || z < GetZ(s2, pos) || z < GetZ(s3, pos)
            AlarmSound();
        elseif isplaying(alarm)
                AlarmStop();
        end
    end
    %% Alarm start sound
    function AlarmSound()
        if not(isplaying(alarm))
            EngineStop();
            play(alarm);      
        end
    ...
    %% Landing function
    function canLand = Land()
        canLand = false;
        if vel < 200 && vel > 100 && rot(1,1) > 0.9166
            canLand = true;
        end
    end
end